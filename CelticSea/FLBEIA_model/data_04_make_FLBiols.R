...
### PJD -- isn't the process of checking the FLStocks all done already in
### data_02_c_stock_objects.R ?? -- so far as I can see, all that's needed here
### is as follows:
...
## save the biols
save(biols, file = file.path("results", "FLBEIA_inputs", "preconditioned", "FLBiols.RData"))

## Save the first data year of any of the biols
data_yrs  <-c(min(sapply(biols, function(x) x@range[["minyear"]])),
