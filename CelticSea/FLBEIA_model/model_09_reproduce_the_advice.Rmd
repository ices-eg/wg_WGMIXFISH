...
## COD reproduce the advice

```{r}
sc_list <- c( "cod.27.7e-k")### name some

fleets.ctrl.list <- vector(mode = "list", length = length(sc_list))

fleets.ctrl.list<- fleets.ctrl

## Non residual fleets
flt_list <- grep("_fleet", names(fleets), value = TRUE, invert = TRUE)

for(f in flt_list) {
   fleets.ctrl.list[["cod.27.7e-k"]][[f]]$effort.model <- "SMFB"
   fleets.ctrl.list[["cod.27.7e-k"]][[f]]$effort.restr <- "cod.27.7e-k"  
   fleets.ctrl.list[["cod.27.7e-k"]][[f]]$restriction  <- "catch" 
}
...
cod.biols <- FLBiols(biols$`cod.27.7e-k`)
cod.biols <- biols$`cod.27.7e-k`

runs  <- FLBEIA(biols = cod.biols, 
              SRs = SRs$`cod.27.7e-k`,
              BDs = NULL,
              fleets = FLFleet(fleets$`cod.27.7e-k`,
              indices = NULL,
              advice = advice,
              covars = NULL,
              main.ctrl = main.ctrl,
              biols.ctrl = biols.ctrl$`cod.27.7e-k`,
...
```{r}
sc_list <- c( "cod.27.7e-k", "had.27.7b-k", "whg.27.7b-ce-k")#,,"min",  "max", "prev") ## name some
...
for(f in flt_list) {
  fleets.ctrl.list[["had.27.7b-k"]][[f]]$effort.model <- "SMFB"
  fleets.ctrl.list[["had.27.7b-k"]][[f]]$effort.restr <- "had.27.7b-k"  
  fleets.ctrl.list[["had.27.7b-k"]][[f]]$restriction  <- "catch" 
}
for(f in flt_list) {
  fleets.ctrl.list[["whg.27.7b-ce-k"]][[f]]$effort.model <- "SMFB"
  fleets.ctrl.list[["whg.27.7b-ce-k"]][[f]]$effort.restr <- "whg.27.7b-ce-k"  
  fleets.ctrl.list[["whg.27.7b-ce-k"]][[f]]$restriction  <- "catch" 
}
