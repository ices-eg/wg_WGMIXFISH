...
This WD is to record a simple FLBEIA set-up for mixed fisheries forecasts in the Celtic Sea. It outlines the following steps:
...
#For now lets just do it for cod
load("results/clean_data/clean_stock_objects/cod.27.7e-k.RData")

#stocks_list <- str_sub(stocks_list, 1, str_length(stocks_list)-6)
##~Some fixes ####

stock@discards.wt[ac(4:7),ac(2019)]    <- stock@discards.wt[ac(4:7),ac(2018)]
stock@discards.wt[ac(7),ac(2017:2019)] <- stock@discards.wt[ac(7),ac(2012)] ## last year of discard rate at age 7
...
# stocks[["cod.27.7e-k"]]@discards.wt[ac(7),ac(2017:2019)] <- stocks[["cod.27.7e-k"]]@discards.wt[ac(7),ac(2012)] ## last year of discard rate at age 7
  ...
  if(all(is.na(stock@discards.wt))) {
    stock@discards.wt <- stock@stock.wt
  }
...
if(any(is.na(stock@discards.wt))) {
    stock@discards.wt[is.na(stock@discards.wt)] <- stock@stock.wt[is.na(stock@discards.wt)]
  }
...
fleets   <- FLFleetsExt(as(stock, "FLFleetExt")); 
names(fleets)
names(fleets@.Data[[1]]@range) <- c("min", "max" , "minyear", "maxyear")
...
We ensure each object has the same year dimensions as FLBEIA references internally from 1 to n years. This is for both the FLBiols and FLFleetsExt.
...
# Q for Paul - here you had just lapply - but this was not right format, I think?
#fleets<-lapply(fleets,window,data.yrs[1],last.yr.sim) # Note: keep as list, as slower to access FLFleetsExt  
# summary(fleets_not_use)
fleets<-FLFleetsExt(lapply(fleets,window,data.yrs[1],last.yr.sim)) 
summary(fleets)

names(fleets) <- "cod.27.7e-k"
```
...
for(i in 1:l.fls){
  ...
  for( j in 1:l.metiers){
    ...
    for( k in 1:l.stks){
      ...
      fleets[[i]]@metiers[[j]]@catches[[k]]@landings.wt[, ac(first.yr.sim:last.yr.sim)]  <- yearMeans(fleets[[i]]@metiers[[j]]@catches[[k]]@landings.wt[,ac(2017)])
      fleets[[i]]@metiers[[j]]@catches[[k]]@discards.wt[, ac(first.yr.sim:last.yr.sim)]  <- yearMeans(fleets[[i]]@metiers[[j]]@catches[[k]]@discards.wt[,ac(2017)])
...
#fleets[["FR_Otter_all"]]@metiers[["OTM_DEF_27.7.e"]]@effshare

#excl_met <- "OTM_DEF_27.7.e"

#mets <- fleets[["FR_Otter_all"]]@metiers@names
#mets <- mets[!mets %in% excl_met]

#fleets[["FR_Otter_all"]]@metiers <- FLMetiersExt(fleets[["FR_Otter_all"]]@metiers[["OTH"]],
#             fleets[["FR_Otter_all"]]@metiers[["OTT_CRU_27.7.g"]],
#             fleets[["FR_Otter_all"]]@metiers[["OTT_CRU_27.7.h"]],
#             fleets[["FR_Otter_all"]]@metiers[["OTT_DEF_27.7.g"]],
#             fleets[["FR_Otter_all"]]@metiers[["OTT_DEF_27.7.h"]],
#             fleets[["FR_Otter_all"]]@metiers[["OTT_DEF_27.7.j"]])
            
#fleets[["FR_Otter_all"]]@metiers@names <- as.character(sapply(fleets[["FR_Otter_all"]]@metiers, function(x) x@name)[])
...
# HAD.sr <- FLSRsim(rec = biols[["HAD"]]@n[1,], ssb = ssb(biols[["HAD"]]), 
#                uncertainty = flq_dims, proportion = flq_dims, model = 'geomean', name = "HAD")
# HAD.sr@params[] <- 265133
# 
# WHG.sr <- FLSRsim(rec = biols[["WHG"]]@n[1,], ssb = ssb(biols[["WHG"]]), 
#                uncertainty = flq_dims, proportion = flq_dims, model = 'geomean', name = "WHG")
# WHG.sr@params[] <- 971263

SRs <- list(`cod.27.7e-k` = COD.sr) #, HAD = HAD.sr, WHG = WHG.sr)
range(SRs[[1]])[4] <- 1980
range(SRs[[1]])[5] <- 2022
```
...
growth.model     <- ifelse(!Baranov, c(`cod.27.7e-k`='ASPG'), c(`cod.27.7e-k` = "ASPG_Baranov")) ; names(growth.model) <- c("cod.27.7e-k")
...
## Or ICES HCR
#ref.pts.COD <- matrix(c(7300, 10300, 0.35),ncol = 1, dimnames = list(c("Blim", "Btrigger", "Fmsy"), c(1)))
#ref.pts.HAD <- matrix(c(6700, 10000, 0.4), ncol = 1, dimnames = list(c("Blim", "Btrigger", "Fmsy"), c(1)))
#ref.pts.WHG <- matrix(c(25000, 35000, 0.52),ncol = 1, dimnames = list(c("Blim", "Btrigger", "Fmsy"), c(1)))
#advice.ctrl <- create.advice.ctrl(stksnames = stks, HCR.models = rep('IcesHCR', length(stks)), ref.pts.COD = ref.pts.COD,
#                                  ref.pts.HAD = ref.pts.HAD, ref.pts.WHG = ref.pts.WHG, first.yr = 1993, last.yr = last.yr.sim)
...
advice$TAC['cod.27.7e-k', ac(2018:2020)] <- c(805, rep(0, 2))
# advice$TAC['HAD', ac(2018:2020)] <- c(6910, rep(6317, 2))
# advice$TAC['WHG', ac(2018:2020)] <- c(22213 - 3443, rep(15841, 2))

kable(reshape2::dcast(as.data.frame(advice$TAC[,ac(2018:2020)]), stocks ~ year, value.var = "data"), caption = "TAC advice in intermediate and TAC year")

names(advice$quota.share) <- stks
#fleets[[1]]@metiers[[1]]@catches[[1]]@name <- "cod.27.7e-k"
...
effort.models    <- rep("fixedEffort",n.fls) ; names(effort.models)<-fls
eff.res <- "prev"
...
c.mod$catch.mod<-sapply(c.mod$values,function(x) {
  if(x %in% c("cod.27.7e-k")) return("CobbDouglasAge") else
...
## Haddock selection is average of 1993 - 2017
## Recalculate here
# load(file.path("..","data", "stocks", "HAD-CS.RData"))
# had.sel <- apply(stock@landings.n / (stock@landings.n + stock@discards.n), 1, mean)
# had.sel[1,] <- 0
# had.sel[8:9,] <- 1
# 
  ...
  for( j in 1:l.metiers){
   ...
   for( k in 1:l.stks){
      ...
      if(fleets[[i]]@metiers[[j]]@catches[[k]]@name == "HAD") {
       fleets[[i]]@metiers[[j]]@catches[[k]]@landings.sel[, ac(first.yr.sim:last.yr.sim)] <- had.sel
      } else {
     fleets[[i]]@metiers[[j]]@catches[[k]]@landings.sel[, ac(first.yr.sim:last.yr.sim)] <- yearMeans(fleets[[i]]@metiers[[j]]@catches[[k]]@landings.sel[,ac(sel.yrs)])
      }
...
################
## QA - equivalent to reproduce the advice
################
...
## All others are already set to SMFB effort.model, so we just need to modify the option

for(fl in names(fleets)) { fleets.ctrl.COD[[fl]][["effort.restr"]] <- 'cod.27.7e-k'}
# for(fl in names(fleets)) { fleets.ctrl.HAD[[fl]][["effort.restr"]] <- 'HAD'}
# for(fl in names(fleets)) { fleets.ctrl.WHG[[fl]][["effort.restr"]] <- 'WHG'}
...
advice.cod <- advice
advice.cod$TAC['cod.27.7e-k',ac(c(2019,2020))] <- c(1055, 0)  # catch int yr, TAC. Used catch here, previously paul used landings. Note that for cod this is peculiarly the landings, as the discards aren't in the assessment
...
#for(fl in names(fleets)) { fleets[[fl]]@effort[] <- 1e12} # ensure not cap limited
...
ss_cod <- fb_cod <- matrix(NA, ncol = 3, nrow = 5, 
                 dimnames = list(metric = c("F", "SSB", "catches", "landings", "discards"), 
                                 year = c(2019:2021)))
ss_cod["F",] <- c(1.13, 0.477, 0)
ss_cod["SSB",] <- c(1181, 1587, 2943)
ss_cod["catches",] <- c(1351,0, NA)
ss_cod["landings",] <- c(1051, 0, NA)
ss_cod["discards",] <- c(300, 0, NA)

units(cod$biols[["cod.27.7e-k"]]@wt) <- "kg"
units(cod$biols[["cod.27.7e-k"]]@n) <- "thousands"

cod$biols[["cod.27.7e-k"]]@range[c("minfbar")] <-  2
cod$biols[["cod.27.7e-k"]]@range[c("maxfbar")] <-  5

cod.flbeia <- bioSum(cod, years = ac(2019:2021))
...
fb_cod["landings",] <- cod.fb$landings
