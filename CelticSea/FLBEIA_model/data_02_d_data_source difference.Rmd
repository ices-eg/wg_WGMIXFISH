...
inter_canum <- inter_canum %>% tidyr::spread(CatchCat, total) %>% mutate("catch" = Landings+Discards) %>% gather(catch_cat, total, 5:7) %>% group_by(Year, Stock, source, unit, catch_cat) %>% summarise(total = sum(total, na.rm=T)) %>% data.frame()
...
Do the totals of of intercatch caton and canum match?
Do these total match that displayed in the adviec sheet? 

All data should be in tonnes. 
...
For HKE- units area in kg in intercatch and tonnes in advice sheet at stock object. But the landings in the canum are much higher then Caton. 
```{r}
ggplot(all_dat[all_dat$stock == "hke.27.3a46-8abd"  ,], aes(year, total, colour = catch_cat)) + geom_line() +facet_wrap(~source) + theme_classic()

ggplot(all_dat[all_dat$stock == "hke.27.3a46-8abd"  ,], aes(year, total, colour = source)) + geom_line() +facet_wrap(~catch_cat) + theme_classic()

ggplot(all_dat[all_dat$stock == "hke.27.3a46-8abd" ,], aes(year, total, colour = source)) + geom_bar(stat="identity") +facet_wrap(~catch_cat) + theme_classic()


newdat_hke <- all_dat[all_dat$stock == "hke.27.3a46-8abd" & all_dat$source %in% c("intercatch canum", "intercatch caton"),]
ggplot(newdat_hke, aes(year, total, colour = source)) + geom_line() +facet_wrap(~catch_cat) + theme_classic()
```
...
ggplot(all_dat[all_dat$stock == "sol.27.7fg"  ,], aes(year, total, colour = source)) + geom_line() +facet_wrap(~catch_cat) + theme_classic()
