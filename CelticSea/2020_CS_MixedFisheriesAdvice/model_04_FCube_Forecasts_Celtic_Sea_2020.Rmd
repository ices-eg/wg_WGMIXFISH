...
gc()
rm(list = ls())
ver <- "04_FCube_Forecasts_Celtic_Sea_2020"
ver.datetime   <- date();
...
SkipIntermedYr  <- FALSE # If we want to skip the intermediate year due to different F assumption
                         # From single-stock
...
#! choice of scenarios
scenarios <- c("min","max","sq_E","val", "had.27.7b-k","whg.27.7b-ce-k", "cod_fmsy")
#scenarios <- c("cod_fmsy")
...
```{r TACrules, message=FALSE, warning=FALSE, tidy = TRUE, results = 'asis'}

# Management Plans for yrTAC  
## We now just use the ones loaded from reproduce the advice directly

## Commission request for a cod Fmsy run
# not in 2019 Mixfish Advice group  ? 
     ctrl.MP.CodMSY <- list()
...
slidingslope["cod.27.7e-k"] <-if(SSB_TACyr["cod.27.7e-k"]< BRPs["cod.27.7e-k","Bmsytrigger"]) { BRPs["cod.27.7e-k","Fmsy"]*(SSB_TACyr["cod.27.7e-k"]/BRPs["cod.27.7e-k","Bmsytrigger"])}   
  ...
  ## additional scenario
  ctrl.MP.CodMSY[["cod.27.7e-k"]] <- fwdControl(data.frame(year=c(yr.now,yr.TAC),val=c(1055,slidingslope[["cod.27.7e-k"]]),quantity=c("catch","f"),rel.year=c(NA,NA)))

  ## Output Control rules
  print(lapply(ctrl.MP, function(x) kable(x@target[c("year","quantity","min","val","max","rel.year")])))
 

  ## Output additional scenario
  print(kable(ctrl.MP.CodMSY[["cod.27.7e-k"]]@target[c("year","quantity","min","val","max","rel.year")], 
              caption = "Additional Cod Fmsy scenario"))
...
for (sc in scenarios) {
  ...
  sc_bis <- sc ## non standard scenarios
...
############################################################
  ...
  # Under Fmsy scenario, modify F target for cod
    if(sc == "cod_fmsy" & YY == ac(now)) {
      f3.ctrl@target.F@.Data[1] <- slidingslope[["cod.27.7e-k"]] 
      sc <- "cod.27.7e-k"  # change back for FCube to recognise
      }
...
##################################################
        ...
        if (!SkipIntermedYr | an(YY) !=now) {   # Can choose to skip intermediate year
          ...
          if(sc_bis == "cod_fmsy") {
            ef.fl <- data.frame(scenario=sc_bis,year=YY,fleet=fleet.names,effort=sapply(fl.f3,function(x) effort(x)[,YY]))
           }
		...
		Ftarg_int <- sapply(dem.st.f3, function(x) fbar(x)[,YY]) } else {
			...
			Ftarg_tac[Ftarg_tac == 0] <- 1e-09 ## fix for sam zero f issue
		 	}
...
tot.nep.f3.landings[no.nep.ass,YY] <- sapply(nep.lst, function(x) nep.st.fwd[[x]]@landings[,YY])[no.nep.ass] * Ratio
   ...
   ## For any top up stocks, we need to raise the discards by DR
    ...
    dem.st.f3[[i]]@catch[,YY]     <- computeCatch(dem.st.f3[[i]][,YY])
           ...
           if(sc_bis != "cod_fmsy") { #Claire there is an issue here, it is with names(partF.land)
            ...
            if(sc_bis == "cod_fmsy") {
            rel.share <- cbind(eval(parse(text=paste('rbind(rel.share[[', paste(seq(length(rel.share)),
            			             collapse=']] ,rel.share[['), ']])', sep=''))),stock=rep(names(partF.land),each=length(unique(rs$country))), year=YY,scenario=sc_bis)
            }
	...
	if(UseSAM) {
	    ...
	    disc[[i]] <- sam_results[sam_results$stock == i & sam_results$year == YY,"discards"]
            ...
            if(sc_bis == "cod_fmsy") {
            allcatch.dataframe <- rbind(allcatch.dataframe,
                                        data.frame(year=YY,sc=sc_bis,stock=rep(st.names,3),
                                                   catch=rep(c("legal landings","discards","overTAC landings"),each=length(st.names)),
                                                   tonnes =as.numeric(tonnes)))
            }
        ...
        BaselineNepNoAss <- sapply(nep.lst, function(x) landings(nep.st.fwd[[x]])[,yr.assess])[no.nep.ass]/sapply(nep.lst, function(x) landings(nep.st.fwd[[x]])[,YY])[no.nep.ass]
            ...
            if (YYp1 <= LastProjectionYear) {
                ...
                dem.st.MP <- lapply(dem.st.f3, function(x) {
                                n. <- name(x)
                                  if(sc_bis == "cod_fmsy" & n. == "cod.27.7e-k") {
                                ctrl. <- ctrl.MP.CodMSY[[n.]]
                                } else {
                                  ctrl. <- ctrl.MP[[n.]] 
                                }
    ...
    ## change the scenario name back for results
    sc <- sc_bis
...
print("--- RESULT TABLE ---")
kable(reshape2::dcast(results, year + value + sc ~ stock, value.var = "data"), caption = "FCube Forecasts")

write.table(res,file = paste(res.path, '/', Run.nameFC, '_ResultsTable.csv', sep = ""))
# this doesn't write out well on csv - for now just copy the R console and paste to excel, make the paging by hand -- who wrote this blasphemy!! (pjd)

#reset ver - 2019 fix
save(results, res.effort, file = paste(res.path, '/', Run.nameFC, options,'_Catch_EffortByScenario.Rdata', sep = "") )
save(allcatch.dataframe, file = paste(res.path, '/', Run.nameFC, options,'_AllCatches_Dataframe.Rdata', sep = "") )

kable(allcatch.dataframe, caption = "All catch dataframe")
```
...
for (st in dem.names) {
  print(ggplot(filter(rs, stock == st), aes(x = scenario, y = share)) + geom_point(aes(shape = factor(year))) +
   facet_wrap(~country)+ theme_bw() + theme(axis.text.x = element_text(angle = -90)) + ggtitle(st) )
  }

#changes in effort by metier
res.effort.met$gear <- unlist(lapply(strsplit(res.effort.met$metier,split="\\."),function(x) x[1]))
res.effort.met$gear <- unlist(lapply(strsplit(res.effort.met$gear, split = "_"), function(x) if(length(x) == 1) paste(x[1]) else paste(x[1], x[2], sep ="_")))
...
kable(reshape2::dcast(eff.gear, gear + scenario ~ year, value.var = "x", fun.aggregate = sum), caption = "Effort by gear")

write.table(res,file = paste(res.path, '/', Run.nameFC, '_EffortMetier.csv', sep = ""))

#  plot with dem and NEP aggregated
  res. <- subset(results,value=="landings" & year==yr.TAC)
  res. <- tapply(res.$data,list(res.$sc,res.$stock),sum,na.rm=T)

  if(nep) {
  ## aggregate Nephrops
  res. <- cbind(res.[,-grep("nep", colnames(res.))], matrix(rowSums(res.[,grep("nep",colnames(res.), value = T)]),ncol=1))
  colnames(res.)[ncol(res.)] <- "nep.27.7bk"
  }

  baseline <- res.[1,]
  res. <- res.[-1,]

  pal <-rainbow(length(names(baseline)))

    foo <- function(x) ceiling(max(x)/1000) * 1000
    barplot(t(res.[c(scenarios),]), col=pal, beside = TRUE,legend = TRUE, ylim=c(0,foo(max(res.,baseline))),
            ylab=paste("predicted catch",yr.TAC),xlab="Fcube scenarios",
            cex.axis = 0.8, cex.lab = 0.6, cex.names = 0.6)
    #main =paste(yr.now,"Fcube landings estimates, dem stocks"), )
for(i in seq_len(ncol(res.))) {
    abline(h=baseline[colnames(res.)[i]], col=pal[i])
}

  deff.pred <- subset(res.effort,year==yr.TAC)
  deff.pred$sc <- as.factor(deff.pred$sc)
  #

print(ggplot(filter(deff.pred, fleet != "OTH_OTH"), aes(x = scenario, y = effort)) + geom_bar(stat = "identity", colour = "grey50") + facet_wrap(~fleet) + ylab("Effort ('000 kwdays)")  + theme_bw() +
            theme(axis.text.x = element_text(angle = -90)) + ggtitle("Fleet effort by scenario"))

# PLOT OF EFFORT CORRESPONDING TO THE VARIOUS SPECIES  - DONE ONLY IF FC.SILENT=FALSE
if(!Fc.silent) {

  chokespecies <- matrix(nrow=length(st.names),ncol=2,dimnames=list(st.names,ac(now:LastProjectionYear)))
  unchokespecies <- matrix(nrow=length(st.names),ncol=2,dimnames=list(st.names,ac(now:LastProjectionYear)))

  #for (YY in now:LastProjectionYear){
   for (YY in LastProjectionYear){
        print(YY)

        eff.st <- effort.stock[["max"]][[ac(YY)]]

        deff.st <- data.frame(matrix(nrow = length(fleets)*length(dem.names), ncol = 4,
                                     dimnames = list(NULL, c('fleet', 'stock', 'effort','st.n'))))
        i <- 1
        for(f in names(fleets)){
            for(s in dem.names){
                deff.st[i,1] <- f
                deff.st[i,2] <- s
                deff.st[i,3] <- eff.st[s,f,1]
                deff.st[i,4] <- which(dem.names==s)
                i <- i+1
            }
        }

        deff.st$stock <- as.factor(deff.st$stock)
        deff.st$st.n <- as.factor(deff.st$st.n)

          print(ggplot(filter(deff.st, fleet != "OTH_OTH"), aes(x = stock, y = effort)) +
            geom_bar(stat = "identity", fill = "grey50") +
              facet_wrap(~fleet, scale = "free") + expand_limits(y = 0)  + theme_bw()+ theme(axis.text.x = element_text(angle = -90)) + ggtitle("Effort corresponding to single-stock quota share, dem stocks"))

#          ggsave(paste(plot.path, Run.nameFC, options, YY,'_StockEffortFleetsFreeScale_dem.png', sep = ""), 'png')

	if(nep) {
	nep.st <- data.frame(matrix(nrow = length(fleets)*length(nep.names), ncol = 4,
                                     dimnames = list(NULL, c('fleet', 'stock', 'effort','st.n'))))
        i <- 1
        for(f in names(fleets)){
            for(s in nep.names){
                nep.st[i,1] <- f
                nep.st[i,2] <- s
                nep.st[i,3] <- eff.st[s,f,1]
                nep.st[i,4] <- which(nep.names==s)
                i <- i+1
            }
        }

        nep.st$stock <- as.factor(nep.st$stock)
        nep.st$st.n  <- as.factor(nep.st$st.n)

          print(ggplot(filter(nep.st, fleet != "OTH_OTH"), aes(x = stock, y = effort)) +
            geom_bar(stat = "identity", fill = "grey50") +
              facet_wrap(~fleet, scale = "free") + expand_limits(y = 0)  + theme_bw()+ theme(axis.text.x = element_text(angle = -90)) + ggtitle("Effort corresponding to single-stock quota share, dem stocks"))

 #         ggsave(paste(plot.path, Run.nameFC, options, YY,'_StockEffortFleetsFreeScale_nep.png', sep = ""), 'png')

	}


          stock.names <- sort(stock.names)
        ...
        print ("nbr of fleets")
        nflchoke <- table(zz_min)
        names(nflchoke) <- stock.names[as.numeric( names(nflchoke))]
        print(nflchoke)

        print(paste("#-- least limiting species - % of",yr.assess, "effort limited by each species in", YY))
        print(round(unchokespecies[,ac(YY)]/sum(unchokespecies[,ac(YY)],na.rm=T),3))
        ...
        save(eff.st,file=file.path(res.path,paste(Run.nameFC, options,'_FleetEffortByStock.Rdata',sep="")))
    }
  }


save.image(file=file.path(res.path,paste(Run.nameFC, options,'_FcubeAllObjects.Rdata',sep="")))
write.csv(allcatch.dataframe,file=file.path(res.path,paste(Run.nameFC, options,'_AllCatchDataframe.csv',sep="")))
...
```{r rename_file}
#load('tmp.RData')
#file.rename(from = paste0(ver, ".html"), to = paste0(ver, options, ".html"))
