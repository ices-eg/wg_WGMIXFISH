...
#fleets=fl.f3; stocks=wg.stock; control=f3.ctrl.YY;silent=Fc.silent
  ...
  #------------------------------------------
   ...
   dat.past$iter <- as.factor(as.character(dat.past$iter))
...
if(length(grep("rev",control@effort.rule,ignore.case=T))>0 | length(grep("val",control@effort.rule,ignore.case=T))>0) {
    ...
    rev.past$iter <- as.factor(as.character(rev.past$iter))
    ...
    eff.past$iter <- as.factor(as.character(eff.past$iter))
    ...
    q.fut$iter <- as.factor(as.character(q.fut$iter))
    ...
    eff.fut$iter <- as.factor(as.character(eff.fut$iter))
