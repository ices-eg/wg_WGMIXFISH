...
IcesRangeHCR <- function(stocks, advice, advice.ctrl, year, stknm,...){
   ...
   # disc.nyears  <- ifelse(is.null(advice.ctrl[[stknm]][['disc.nyears']]), wts.nyears, advice.ctrl[[stknm]][['disc.nyears']])
    ...
    if(ageStruct == TRUE){
      if(any(stk@catch[,tail(dimnames(stk@catch)$year,3)]<1e-2)){
        stk <- FLBEIA:::stf_correctSel(stk, nyears = 3, wts.nyears = wts.nyears, fbar.nyears = fbar.nyears, f.rescale = f.rescale) #, disc.nyrs = disc.nyears)
      }else{
         stk <- stf(stk, nyears = 3, wts.nyears = wts.nyears, fbar.nyears = fbar.nyears, f.rescale = f.rescale) #, disc.nyrs = disc.nyears)
      }
    }else{
       stk <- FLBEIA:::stfBD(stk, nyears = 3, wts.nyears = 3, fbar.nyears = 3)
    }
   ...
   # if(dim(stk@m)[1] == 1)    harvest(stk) <- stk@catch.n/stk@stock.n
    ...
    Fmax <- ifelse(b.pos == 1 & ref.pts["Ftarg",] > ref.pts["Fmsy",], ref.pts["Fmsy",], ref.pts["Ftarg",])
    Ftg <- ifelse(b.pos == 0, 0, ifelse(b.pos == 1, Fmax*b.datyr/ref.pts[ 'Btrigger',], Fmax))
    ...
    for(i in 1:iter){
        ...
        if(dim(stki@m)[1] > 1){
          ...
          stki.tmp <- FLBEIA:::fwdBD(stki, fwd.ctrl, growth.years)
          ...
          fwd.ctrl@target[k,'val'] <- Ftg
          fwd.ctrl@trgtArray[k, 'val',] <- Ftg
          stki <- FLBEIA:::fwdBD(stki, fwd.ctrl, growth.years)
        }
