...
setGeneric("FLFcube_FIDES", function(fleets,stocks,control,fides,...){
        standardGeneric("FLFcube_FIDES")
})

setMethod("FLFcube_FIDES", signature(fleets="FLFleets",stocks="FLStocks",control="FLFcube.control",fides="data.frame"),
function(fleets,stocks, control,fides,silent=TRUE){
...
if(length(grep("rev",control@effort.rule,ignore.case=T))>0 | length(grep("val",control@effort.rule,ignore.case=T))>0) {
    ...
    fcube.res <- fcube_FIDES.main(dat.past=dat.past, eff.past=eff.past, dat.fut=dat.fut,eff.fut=eff.fut, control=control,fides=fides)
    ...
    if (!silent) ret. <- list(fleets=res,effort.stock=fcube.res[[2]],value.weight=fcube.res[[3]],ChokeCategory1=fcube.res[[4]]) else 
      ret. <- res
    return(ret.)
...
fcube_FIDES.main <- function(dat.past, eff.past, dat.fut, eff.fut, control, fides) {
...
## Prognosis for next year
  ...
  ################
  ## FIDES Choke category 1############
  ## if the sum of nextF by fleet by country is lower than the sum of pastF by fleet and country 
  #(i.e. next TAC is not limiting for the country with regards to current effort)
  #then the stock does not enter the "Min" calculation
  rel.stab.country <- fides[fides$stock!="NEP-NS",] 
  rel.stab.country <- tapply(rel.stab.country$qshareInitial, list(rel.stab.country$stock, rel.stab.country$country), sum, na.rm=T)
  rel.stab.country[is.na(rel.stab.country)] <- 0
  nextF.country <- sweep(rel.stab.country, 1,targ[-grep("NEP",dimnames(targ)[[1]])], FUN="*")
  #compare with sum of metier
  pastF.country <-as.data.frame.table(sweep(nextQ,2:3,mean.past.eff,FUN="*"))
  pastF.country$country <- as.character(substr(pastF.country$Var2,1,2))
  pastF.country[pastF.country$country %in% c("EN","SC"),"country"] <- "UK"
  pastF.country <- pastF.country[!pastF.country$country=="OT",]
  pastF.country$Var1 <- as.character(pastF.country$Var1)
  pastF.country <- pastF.country[-grep("NEP",pastF.country$Var1),]
  pastF.country <- tapply(pastF.country$Freq, list(pastF.country$Var1,pastF.country$country),sum,na.rm=T)
  #identifying where there is a risk of choke
 #YV 2019 reverse the TRUE/FALSE
   ChokeCategory1 <-array(FALSE, dim=c(length(n.st),length(n.fl), n.it), dimnames=list(n.st, n.fl, seq(n.it)))
  #ChokeCategory1 <-array(TRUE, dim=c(length(n.st),length(n.fl), n.it), dimnames=list(n.st, n.fl, seq(n.it)))
  
   for (i in n.fl) {
      ct <- substr(i,1,2)
      if(ct %in% c("EN","SC")) ct <- "UK"
      if (ct %in% dimnames(nextF.country)[[2]]){
    for (k in n.st) {
      if (k %in% dimnames(nextF.country)[[1]]){
        #if ((nextF.country[k,ct]>pastF.country[k,ct])) ChokeCategory1[k,i,] <- FALSE
        ## YV reverse the TRUE/FALSE
        if ((nextF.country[k,ct]<pastF.country[k,ct])) ChokeCategory1[k,i,] <- TRUE
      }}}}
...
for (j in seq(n.it)) {
  ...
  if(length(grep("max",rule[i,j],ignore.case=T))>0)
    ...
    if(length(grep("min",rule[i,j],ignore.case=T))>0){
      #### YV 2019 fix for fleets where no chockeCategory1==T then set the effort to max Effort
      
      # if (isTRUE(any(ChokeCategory1[,i,j]))) man.Effort[i,j] <- min(nextEffort.stock[which(ChokeCategory1[,i,j]),i,j],na.rm=T) else
      #                                        man.Effort[i,j] <- min(nextEffort.stock[,i,j],na.rm=T)
      
      if (isTRUE(any(ChokeCategory1[,i,j]))){
        if (min(nextEffort.stock[which(ChokeCategory1[,i,j]),i,j],na.rm=T)!=Inf){
          man.Effort[i,j] <- min(nextEffort.stock[which(ChokeCategory1[,i,j]),i,j],na.rm=T)  ## set the minimum effort if a chocke species exists
        } else{
          man.Effort[i,j] <-mean.past.eff[i,j]    ## otherwise set the sq_E
        }
      }  else {
        man.Effort[i,j] <- min(nextEffort.stock[,i,j],na.rm=T)
      }
      ...
      #### end fix YV 2019
      #if(is.infinite(man.Effort[i,j]))     man.Effort[i,j] <- mean.past.eff[i,j]
    }
...
return(res=list(final=man.Effort,effort.stock=nextEffort.stock,value.weight=value.weight,ChokeCategory1=ChokeCategory1))
}
