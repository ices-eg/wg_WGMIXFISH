...
create.fleets.data_tmp <- function(yrs,ns,ni,fls.data,stks.data){
  ...
  list.stks.flqa <-  FLBEIA:::create.list.stks.flqa(stks,yrs,ni,ns,list.stks.unit,list.stks.age)  
  list.stks.flq  <-  FLBEIA:::create.list.stks.flq(stks,yrs,ni,ns,list.stks.unit)  
    ...
    fleet <- FLFleetExt(name = nmfl, 
     ...
     #-----------------------------------------------------------------------------
      ...
      if(is.FLQuant(fl.effort)){
        log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(fl.effort,fleet@effort[,hist.yrs]),2)
      ...
      if(!is.na(fl.fcost)){
        fl.fcost <- fl.fcost[[1]]
        log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(fl.fcost,fleet@fcost[,hist.yrs]),2)
      ...
      if(!is.na(fl.capacity)){
        ...
        log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(fl.capacity,fleet@capacity[,hist.yrs]),2)
      ...
      if(!is.na(fl.crewshare)){
        ...
        log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(fl.crewshare,fleet@crewshare[,hist.yrs]),2)
      ...
      for (j in 1:n.fl.met){ #loop metier
       ...
       if(length(fl.met.vcost)==0) fl.met.vcost  <- NA
        ...
        if(is.FLQuant(fl.met.effshare)){
          log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(fl.met.effshare, fleet@metiers[[nmfl.met]]@effshare[,hist.yrs]),2)
        ...
        if(!is.na(fl.met.vcost)){
          ...
          log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(fl.met.vcost,fleet@metiers[[nmfl.met]]@vcost[,hist.yrs]),2)
        ...
        for(k in 1:n.fl.met.stks){
          ...
          log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(landings.n,fl.met.stk.landings.n[,hist.yrs]),1:2)
          ...
          log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(landings.wt,fl.met.stk.landings.wt[,hist.yrs]),1:2)
          ...
          if(!all(is.na(discards.n))){
            ...
            log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(discards.n,fl.met.stk.discards.n[,hist.yrs]),1:2)
          ...
          if(!all(is.na(discards.wt))){
            ...
            log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(discards.wt,fl.met.stk.discards.wt[,hist.yrs]),1:2)
          ...
          if(!all(is.na(price))){
            price <- price[[1]]
            log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(price,fl.met.stk.price[,hist.yrs]),1:2)
          ...
          if(all(is.na(alpha)) || all(is.na(beta)) || all(is.na(catch.q))){
            ...
            CD_param <- FLBEIA:::calculate.CDparam(stk.n, fl.met.stk.landings.n,fl.met.stk.discards.n,
            ...
            log.dim <- FLBEIA:::equal.flq.Dimnames(lflq=list(alpha,beta,catch.q,flqa.stk[,hist.yrs]),1:2) ###
